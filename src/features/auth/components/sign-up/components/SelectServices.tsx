...
import { useRequests, useSubcategories, useProviderRequirements } from "@/common/hooks";
import {
  getCategoryIcon,
  getCategoryColor,
...
const SelectServices = () => {
  ...
  const { generalRequirements, isLoading: loadingRequirements } = useProviderRequirements();
  ...
  const handleNext = () => {
    // Guardar las categorías y subcategorías seleccionadas en el contexto
    updateRegistrationData({ selectedCategories });
  ...
  const availableCategories = categories.filter(
    cat => !selectedCategories.some(sc => sc.categoryId === cat.id)
  );

  // Categorías con requisitos adicionales (para mostrar en la sección de requisitos)
  const categoriesWithExtraRequirements = categories.filter(
    (c): c is Request & { extra_requirements: string } =>
      !!c.extra_requirements?.trim()
  );
      ...
      <SafeAreaView className="flex-1">
        ...
        <ScrollView className="flex-1 px-5 pt-6">
          ...
          <Text className="text-sm text-gray-600 mb-4">
            Puedes agregar hasta {MAX_CATEGORIES} categorías. Selecciona las
            subcategorías para cada una.
          </Text>

          {/* Requisitos para proveedores (general + por categoría) */}
          {(generalRequirements || categoriesWithExtraRequirements.length > 0) && (
            <View className="mb-4 p-4 bg-amber-50 rounded-xl border border-amber-200">
              <Text className="text-sm font-semibold text-amber-900 mb-2">
                Requisitos para registrarte como proveedor
              </Text>
              {!loadingRequirements && generalRequirements && (
                <Text className="text-sm text-amber-800 mb-2">
                  {generalRequirements}
                </Text>
              )}
              {categoriesWithExtraRequirements.length > 0 && (
                <>
                  <Text className="text-sm font-medium text-amber-900 mt-2 mb-1">
                    Requisitos adicionales por categoría:
                  </Text>
                  {categoriesWithExtraRequirements.map(cat => (
                    <Text
                      key={cat.id}
                      className="text-sm text-amber-800 mb-1 pl-2"
                    >
                      • <Text className="font-medium">{cat.name}:</Text>{" "}
                      {cat.extra_requirements}
                    </Text>
                  ))}
                </>
              )}
              <Text className="text-sm text-amber-800 mt-2 pt-2 border-t border-amber-200">
                Si eliges una categoría profesional (Enfermería, Abogados, Médicos, Contadores, etc.), en el siguiente paso deberás subir tu carnet o certificado profesional.
              </Text>
            </View>
          )}
              ...
              {selectedCategories.length < MAX_CATEGORIES && (
                <View className="mb-6">
                  <Text className="text-base font-semibold text-gray-700 mb-3">
                    Agregar categoría ({selectedCategories.length}/
                    {MAX_CATEGORIES})
                  </Text>
              ...
              {selectedCategories.length > 0 && (
                <View className="mt-4 p-4 bg-blue-50 rounded-xl mb-4">
                  ...
                  {selectedCategories.map(sc => (
                    ...
                    >
                      • {sc.categoryName}: {sc.selectedSubcategories.length}{" "}
                      subcategoría
                      {sc.selectedSubcategories.length !== 1 ? "s" : ""}
                    </Text>
                  ))}
                </View>
              )}
        ...
        <View className="flex-row items-center flex-1">
          ...
          <View className="flex-1">
            ...
            <Text className="text-xs text-gray-500">
              {selectedCategory.selectedSubcategories.length} subcategoría
              {selectedCategory.selectedSubcategories.length !== 1
                ? "s"
                : ""}{" "}
              seleccionada
              {selectedCategory.selectedSubcategories.length !== 1 ? "s" : ""}
            </Text>
          </View>
        </View>
        <TouchableOpacity
      ...
      {isLoading ? (
        ...
        <View className="mt-2">
          {subcategories.map((subcategory: Subcategory) => {
            const isSelected = selectedCategory.selectedSubcategories.includes(
              subcategory.id
            );
